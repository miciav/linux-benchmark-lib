...
Esempio:
```bash
VM_NAME=hpl-build-amd64 VM_ARCH=x86_64 bash linux_benchmark_lib/plugins/hpl/build_hpl_vm.sh
```
...
- Uso:
```bash
bash linux_benchmark_lib/plugins/hpl/test_hpl_vm.sh /path/to/hpl_2.3-1_*.deb
```
...
- Uso:
```bash
TARGET_ARCH=linux/amd64 bash linux_benchmark_lib/plugins/hpl/build_hpl_docker_cross.sh
```
...
- Passa il percorso del `.deb` nel contesto con `--build-arg HPL_DEB=...` (default: `hpl_2.3-1_arm64.deb` o `hpl_2.3-1_amd64.deb`); usa percorsi relativi al contesto (es. `linux_benchmark_lib/plugins/hpl/hpl_2.3-1_arm64.deb`).
- Se vuoi forzare l’architettura, usa `docker build --platform linux/arm64` o `--platform linux/amd64`.
- Esempi:
  ```bash
  docker build --platform linux/arm64  -f linux_benchmark_lib/plugins/hpl/Dockerfile.arm   --build-arg HPL_DEB=linux_benchmark_lib/plugins/hpl/hpl_2.3-1_arm64.deb .
  docker build --platform linux/amd64  -f linux_benchmark_lib/plugins/hpl/Dockerfile.amd64 --build-arg HPL_DEB=linux_benchmark_lib/plugins/hpl/hpl_2.3-1_amd64.deb .
  ```
...
- Esempio:
  ```bash
  DOCKER_USER=youruser bash linux_benchmark_lib/plugins/hpl/push_hpl_images.sh
  ```
