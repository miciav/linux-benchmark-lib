...
### k6 logging/output
- `k6_log_stream` (bool, default true): stream k6 log output via SSH.
- `k6_outputs` (list[str], default empty): extra `--out` outputs (e.g. Loki).
- `k6_tags` (map, default empty): extra k6 tags merged with run_id/component/workload/repetition.
...
- `grafana.api_key` (str, optional): Grafana API key.
- `grafana.org_id` (int, default 1): Grafana organization id.
- Default dashboard is imported from `lb_plugins/plugins/dfaas/grafana/dfaas-dashboard.json`.
- Install/uninstall scripts live under `lb_plugins/plugins/dfaas/scripts/`.
