...
class K6Runner:
    ...
    def build_script(
        ...
        lines: list[str] = [
            ...
            "",
            "// Shared metrics with 'dfaas_' prefix for efficient Prometheus queries",
            "// These use scenario tags (function) for per-function breakdown in Prometheus",
            'const dfaas_success_rate = new Rate("dfaas_success_rate");',
            'const dfaas_latency = new Trend("dfaas_latency");',
            'const dfaas_request_count = new Counter("dfaas_request_count");',
            "",
        ]
        ...
        for name, rate in sorted(config_pairs, key=lambda pair: pair[0]):
            ...
            # Per-function metrics for local summary parsing
            lines.extend(
                [
                    ...
                    "  // Update both per-function and shared metrics",
                    f"  success_rate_{metric_id}.add(ok);",
                    f"  latency_{metric_id}.add(res.timings.duration);",
                    f"  request_count_{metric_id}.add(1);",
                    "  dfaas_success_rate.add(ok);",
                    "  dfaas_latency.add(res.timings.duration);",
                    "  dfaas_request_count.add(1);",
