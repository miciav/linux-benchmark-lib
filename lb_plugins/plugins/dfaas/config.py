...
class GrafanaConfig(BaseModel):
    ...
    url: str = Field(
        default="http://localhost:3000", description="Grafana base URL"
    )
    api_key: str | None = Field(
        default=None, description="Grafana API key (optional)"
    )
    ...
    def _apply_env_overrides(cls, values: Any) -> Any:
        ...
        env_enabled = _parse_bool_env(os.environ.get("LB_GRAFANA_ENABLED"))
        env_url = os.environ.get("LB_GRAFANA_URL")
        env_api_key = os.environ.get("LB_GRAFANA_API_KEY")
        env_org = _parse_int_env(os.environ.get("LB_GRAFANA_ORG_ID"))
