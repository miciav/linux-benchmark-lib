...
def test_ansible_task_passes_loki_env():
    ...
    playbook_path = Path("lb_controller/ansible/roles/workload_runner/tasks/run_single_rep.yml")
    ...
    for item in content:
        if "block" in item and item.get("name") == "Run workload repetition via LocalRunner":
            for subtask in item["block"]:
                # Identify by the command usage
                if "lb_runner.services.async_localrunner" in str(subtask.get("ansible.builtin.command", "")):
                    runner_task = subtask
                    break
