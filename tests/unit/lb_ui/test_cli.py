...
def test_plugins_enable_disable_persists_config(
    ...
    result = runner.invoke(cli.app, ["plugin", "enable", "stress_ng"])
    ...
    result = runner.invoke(cli.app, ["plugin", "disable", "stress_ng"])
...
def test_doctor_controller_uses_checks(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        cli.doctor_service,
        "check_controller",
        lambda: MockDoctorReport(total_failures=0),
    )
...
def test_doctor_local_tools_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        cli.doctor_service,
        "check_local_tools",
        lambda: MockDoctorReport(total_failures=1),
    )
...
@pytest.mark.unit_ui
def test_run_command_saves_ui_stream_log(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
):
    cli = _load_cli(monkeypatch, tmp_path)
    runner = CliRunner()
