...
def test_dashboard_log_slicing_respects_height():
    ...
    console = SimpleNamespace(size=SimpleNamespace(height=20, width=80))
    dash = RichDashboard(
...
def test_headless_adapter_progress_and_dashboard():
    ...
    assert isinstance(dash, DashboardHandleAdapter)


def test_headless_dashboard_records_logs():
    ui = HeadlessUI()
    viewmodel = build_dashboard_viewmodel([], _fake_journal())
    dashboard = ui.dashboard.create(viewmodel)
    with dashboard.live():
...
def test_threaded_dashboard_handle_dispatches_calls():
    ...
    dashboard = DashboardHandleAdapter(sink, threaded=True)
    with dashboard.live():
...
def test_dashboard_handle_dispatches_unthreaded_calls():
    class _RecordingDashboard:
        def __init__(self) -> None:
            self.calls: list[tuple[str, str | None]] = []
    ...
    sink = _RecordingDashboard()
    dashboard = DashboardHandleAdapter(sink, threaded=False)
    with dashboard.live():
