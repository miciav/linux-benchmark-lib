...
def test_installer_handles_archive_and_directory(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    dummy_plugin_path: Path,
    source: str,
):
...
def test_install_from_git_url(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, dummy_plugin_path: Path
):
...
def test_uninstall_and_config_cleanup(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, dummy_plugin_path: Path
):
    ...
    assert (
        not (plugin_dir / "dummy.py").exists()
        or not (plugin_dir / installed_name).exists()
    )
