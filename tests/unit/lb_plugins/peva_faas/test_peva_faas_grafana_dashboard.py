...
DASHBOARD_PATH = Path("lb_plugins/plugins/peva_faas/grafana/peva_faas-dashboard.json")
...
def test_default_dashboard_json_valid() -> None:
    ...
    assert data["title"] == "PEVA-faas Overview"
...
def test_dashboard_uses_peva_faas_metrics() -> None:
    """Dashboard queries should use efficient peva_faas_* metric names."""
    data = _load_dashboard()
    panels = data.get("panels", [])
    ...
    assert len(peva_faas_panels) == 4

    for panel in peva_faas_panels:
...
def test_dashboard_template_queries_use_peva_faas_metrics() -> None:
    """Template variable queries should use peva_faas_* metrics for efficiency."""
    data = _load_dashboard()
    templating = data.get("templating", {}).get("list", [])

    run_id_var = next((t for t in templating if t.get("name") == "run_id"), None)
    assert run_id_var is not None
    assert "peva_faas_success_rate" in run_id_var.get("query", "")
    ...
    assert "peva_faas_success_rate" in function_var.get("query", "")
    assert "__name__" not in function_var.get("query", "")


def test_dashboard_has_required_panels() -> None:
    """Dashboard should have all required panels for PEVA-faas monitoring."""
    data = _load_dashboard()
    panels = data.get("panels", [])
