...
def test_request_sends_headers_and_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_urlopen(
        req: Request, timeout: float | None = None
    ) -> DummyResponse:
...
def test_request_handles_expected_http_error(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_urlopen(
        req: Request, timeout: float | None = None
    ) -> DummyResponse:
...
def test_upsert_datasource_updates_existing(monkeypatch: pytest.MonkeyPatch) -> None:
    client = GrafanaClient(base_url="http://grafana")
    request_mock = MagicMock(
        side_effect=[(200, {"id": 9}), (200, {"ok": True})]
    )
