...
def test_dashboard_template_queries_use_dfaas_metrics() -> None:
    ...
    run_id_var = next(
        (t for t in templating if t.get("name") == "run_id"), None
    )
    ...
    function_var = next(
        (t for t in templating if t.get("name") == "function"), None
    )
