...
def test_system_info_write_failure_is_ignored(mocker, tmp_path):
    ...
    mocker.patch("lb_runner.services.system_info.collect_system_info").return_value = MagicMock(to_dict=lambda: {})
    mocker.patch("lb_runner.services.storage.write_system_info_artifacts", side_effect=RuntimeError("fail"))
    registry.create_generator.return_value = MagicMock(_is_running=False, stop=lambda: None, get_result=lambda: {})
