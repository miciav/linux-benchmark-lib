...
def test_char_run_benchmark_local_flow(
    ...
    cfg = BenchmarkConfig(
        ...
        remote_hosts=[
            RemoteHostConfig(
                name="localhost", address="127.0.0.1", user="root", become=False
            )
        ],
    )
    ...
    context = run_service.build_context(cfg, tests=["stress"], execution_mode="local")

    run_service.execute(context=context, run_id="test_run", ui_adapter=mock_ui)
...
def test_char_run_benchmark_remote_flow(
    ...
    cfg = BenchmarkConfig(
        ...
        remote_hosts=[
            RemoteHostConfig(
                name="host1", address="192.168.1.10", user="user", become=True
            )
        ],
    )
    ...
    # 3. Execute
    context = run_service.build_context(cfg, tests=["stress"], execution_mode="remote")

    run_service.execute(context=context, run_id="test_run_remote", ui_adapter=mock_ui)
