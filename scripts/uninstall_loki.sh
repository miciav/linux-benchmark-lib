...
print_usage() {
  cat <<EOF
Usage: uninstall_loki.sh [--remove-data|--keep-data]
...
case "${METHOD}" in
  brew)
    if command -v brew >/dev/null 2>&1; then
      brew services stop grafana/tap/loki >/dev/null 2>&1 || brew services stop loki >/dev/null 2>&1 || true
      brew uninstall grafana/tap/loki >/dev/null 2>&1 || brew uninstall loki >/dev/null 2>&1 || true
    else
