...
# Convenience: prefer uv run, but keep commands readable
uvrun() { uv run "$@"; }
...
run_step "1/12 System + Python info" bash -lc "
  uvrun python -V > \"$OUT/python_version.txt\" 2>&1 || true
  uvrun python -c \"import platform; print(platform.platform())\" > \"$OUT/platform.txt\" 2>&1 || true
...
run_step "3/12 Ruff (lint)" bash -lc "
  cd \"$ROOT\" || exit 1
  uvrun ruff check . --output-format=concise > \"$OUT/ruff_check.txt\" 2>&1 || true
"

run_step "4/12 Ruff (stats)" bash -lc "
  cd \"$ROOT\" || exit 1
  uvrun ruff check . --statistics > \"$OUT/ruff_stats.txt\" 2>&1 || true
"

run_step "5/12 Type checking" bash -lc "
  cd \"$ROOT\" || exit 1
  if uvrun python -c \"import importlib.util,sys; sys.exit(0 if importlib.util.find_spec('pyright') else 1)\"; then
    uvrun pyright > \"$OUT/pyright.txt\" 2>&1 || true
  else
    uvrun mypy . > \"$OUT/mypy.txt\" 2>&1 || true
  fi
"

run_step "6/12 Complexity (radon/xenon/lizard)" bash -lc "
  cd \"$ROOT\" || exit 1
  uvrun radon cc -s -a \"$TARGET\" > \"$OUT/radon_cc.txt\" 2>&1 || true
  uvrun radon mi -s \"$TARGET\" > \"$OUT/radon_mi.txt\" 2>&1 || true
  uvrun xenon --max-absolute B --max-modules B --max-average A \"$TARGET\" > \"$OUT/xenon.txt\" 2>&1 || true
  uvrun lizard -l python -C 10 -L 200 \"$TARGET\" > \"$OUT/lizard.txt\" 2>&1 || true
"

run_step "7/12 Dead code (vulture)" bash -lc "
  cd \"$ROOT\" || exit 1
  uvrun vulture \"$TARGET\" --min-confidence 80 > \"$OUT/vulture.txt\" 2>&1 || true
"

run_step "8/12 Dependency hygiene (deptry)" bash -lc "
  cd \"$ROOT\" || exit 1
  uvrun deptry . > \"$OUT/deptry.txt\" 2>&1 || true
"

run_step "9/12 Security (pip-audit/bandit/semgrep)" bash -lc "
  cd \"$ROOT\" || exit 1
  uvrun pip-audit > \"$OUT/pip_audit.txt\" 2>&1 || true
  uvrun bandit -r \"$TARGET\" -q > \"$OUT/bandit.txt\" 2>&1 || true
  uvrun semgrep --config auto \"$TARGET\" > \"$OUT/semgrep_auto.txt\" 2>&1 || true
"

# --- Grimp: robust block (no tee, no fragile pipes) ---
run_step "10/12 Import graph + cycles (grimp)" bash -lc "
  cd \"$ROOT\" || exit 1
  uvrun python - <<'PY' > \"$OUT/grimp_cycles.txt\" 2>&1 || true
import os
import sys
...
run_step "11/12 Import diagram (pydeps)" bash -lc "
  ...
  if command -v dot >/dev/null 2>&1; then
    uvrun pydeps \"$TARGET\" --max-bacon=2 --noshow --show-deps --output \"$OUT/pydeps.svg\" > \"$OUT/pydeps.txt\" 2>&1 || true
  else
...
run_step "12/12 Tests + coverage (if tests exist)" bash -lc "
  ...
  if [[ -d \"$ROOT/tests\" ]]; then
    uvrun pytest -q --disable-warnings --maxfail=1 --cov=\"$TARGET\" --cov-report=term-missing > \"$OUT/pytest_cov.txt\" 2>&1 || true
  else
