...
- Create env and install: `uv sync --all-extras`. Do **not** use `uv pip install -e ".[dev]"` for development: the lint/type/security toolchain (ruff, mypy, pre-commit, bandit, semgrep, deptry, yamllint, ansible-lint, import-linter) is declared in `[dependency-groups].dev`, which `uv sync` installs and the `dev` extra does not. Then enable the hooks with `uv run pre-commit install`.
